          ...
          if (prev_line_token.is_symbol && token.is_symbol == false) ||
            ...
            if number.is_symbol_adjacent?(symbol)
              ...
              if number.is_gear_symbol_adjacent?(symbol)
                symbol.add_gear_ratio_term(number)
              end
              puts "\n"
            end
          end
...
class Token
  attr_accessor :value, :is_symbol, :start_pos, :end_pos, :line_number, :gear_terms
  attr_reader :id
  ...
  def dump
    ...
    if @gear_terms.length == 2
      @gear_terms.each do |term|
        puts "\t"
        term.dump
      end
    end
  ...
  def add_gear_ratio_term(token)
    unless is_gear_symbol?
      raise "the argument to this method is NOT a gear"
    end

    unless gear_terms.any? {|tuple| tuple.id == token.id}
      gear_terms.append(token)
    end
  end


  def is_symbol_adjacent?(symbol)
    unless symbol.is_symbol
      raise "the argument to this method is not a symbol"
    end
...
gears.each do |gear|
  gear.dump
  term1 = gear.gear_terms[0]
  term2 = gear.gear_terms[1]

  total = total + (term1.value.to_f * term2.value.to_f)
end

puts "\n\nGear Total: #{total.to_i}\n"