...
class Mapping
  attr_accessor :source_start, :source_end, :dest_start, :range, :dest_end
  def initialize(input)
    ...
    @dest_end = (@dest_start + @range).to_i
  end
  ...
  def has_source?(destination)
    if @dest_start <= destination && location <= @dest_end
      return true
    end
  ...
  def get_source(destination)
    unless has_destination?(destination)
      raise "Mapping does not cover the supplied start value"
    end
...
# sample first and last of each input range (seed number)
  # determine where those ranges land on the oppposite end
